...
public class BoardDAO {
	...
	// 데이터베이스의 Board테이블에서 입력받은 id에 해당하는 레코드의 모든 컬럼을 선택해서 반환하는 메서드
	...
	public int insert(InsertBoardDTO dto) {
		...
		Connection connection = null;
		// statement : 완벽한 쿼리를 받아서 실행?
		// PreparedStatement : 동적으로 SQL문의 값을 지정 할 수 있도록 함
		PreparedStatement preparedStatement = null;
	...
	// 데이터베이스에서 Board테이블의 레코드 중 입력받은 id에 해당하는 레코드의 title, content를 입력받은 값으로 수정
	// SQL : UPDATE Board SET boardTitle = ?, boardContent = ? WHERE id = ?;
	// 예상되는 반환 값 : 0 or 1      (다른 경우엔 다수의 수가 올 수 있음)
	public int update(UpdateBoardDTO dto) {
		...
		try {
			...
			preparedStatement.setString(1, dto.getBoardTitle());
			preparedStatement.setString(2, dto.getBoardContent());
			preparedStatement.setInt(3, dto.getId()); // 완성
