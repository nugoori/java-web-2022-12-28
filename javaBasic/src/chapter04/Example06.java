...
// Thread 클래스를 상속받아 Thread를 만드는 방법
class Thread1 extends Thread{
	// run을 Override 해서 작업할 내용을 구현
	public void run() {
		for ( int i = 0; i < 20; i++) { 
			System.out.print("*");
			// Thread/yield() : 자신의 시간을 다음 순번의 Thread에 양보
			Thread.yield();
...
class CountDown extends Thread{
	public void run() {
		// Thread.sleep() : 지정한 시간 동안 해당 스레드를 Blocked 상태로 만듦
		for (int i = 10; i > 0; i--) {
...
public class Example06 {
	...
	public static void main(String[] args) {
		...
		//Thread 클래스의 setPriority 메서드로 우선 순위 지정
		// 우선순위를 지정하지 않으면 기본 값으로 5를 가짐
